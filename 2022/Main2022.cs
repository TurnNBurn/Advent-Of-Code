public class AdventOfCode2022
{

    public static void run()
    {
        AdventOfCode2022Day1.run();
        AdventOfCode2022Day2.run();
        AdventOfCode2022Day3.run();
        AdventOfCode2022Day4.run();
        AdventOfCode2022Day5.run();
        AdventOfCode2022Day6.run();
        AdventOfCode2022Day7.run();
        AdventOfCode2022Day8.run();
        AdventOfCode2022Day9.run();
        AdventOfCode2022Day10.run();
        AdventOfCode2022Day11.run();
        AdventOfCode2022Day12.run();
        AdventOfCode2022Day13.run();
        AdventOfCode2022Day14.run();
        AdventOfCode2022Day15.run();
        AdventOfCode2022Day16.run();
        AdventOfCode2022Day17.run();
    }
}